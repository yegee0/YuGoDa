  ...
  return (
    <motion.div key="orders" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-2 bg-white dark:bg-[#111] border border-gray-100 dark:border-white/5 rounded-xl p-1 shadow-sm flex-wrap">
          {FILTERS.map(f => (
            <button
              key={f}
      ...
      {displayOrders.length === 0 ? (
        ...
        <div className="space-y-3">
          {displayOrders.map(order => (
            <div key={order.id} className="bg-white dark:bg-[#111] border border-gray-100 dark:border-white/5 rounded-2xl p-5 shadow-sm space-y-0">
                  ...
                  <div className="flex gap-2">
                    {order.status === 'pending' && (
                      <>
                        <button
                          ...
                          className="px-4 py-2 bg-[#1A4D2E] text-white rounded-xl text-xs font-bold hover:bg-[#133b23] transition-colors"
                        >
                          Approve
                        </button>
                        <button
                          onClick={() => onUpdateOrderStatus(order.id, 'cancelled')}
                          className="px-4 py-2 bg-red-50 dark:bg-red-900/20 text-red-500 rounded-xl text-xs font-bold hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors"
                        >
                          Deny
                        </button>
                      </>
                    ...
                    {order.status === 'ready' && (
                      <button
                        ...
                        className="px-4 py-2 bg-purple-50 dark:bg-purple-900/20 text-purple-600 rounded-xl text-xs font-bold hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors"
                      >
                        Next →
                      </button>
                    )}
              ...
              {(order.status === 'preparing' || order.status === 'ready') && (
                <div className="pt-3 mt-3 border-t border-gray-50 dark:border-white/5 flex flex-wrap gap-3">
              ...
              {order.status === 'delivering' && (
                <div className="pt-3 mt-3 border-t border-gray-50 dark:border-white/5 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className={`w-28 px-3 py-2 text-center tracking-[0.5em] font-black text-base bg-gray-50 dark:bg-white/5 border rounded-xl focus:outline-none transition-colors text-gray-900 dark:text-white ${
                        codeErrors[order.id] ? 'border-red-400' : 'border-gray-200 dark:border-white/10 focus:border-[#1A4D2E]'
                      }`}
                    />
                    <button
              ...
              {!['preparing', 'ready', 'delivering'].includes(order.status) && (order.estimatedPickupTime || order.trackingNotes) && (
                <div className="pt-3 mt-3 border-t border-gray-50 dark:border-white/5 flex flex-wrap gap-3 text-xs text-gray-400">
